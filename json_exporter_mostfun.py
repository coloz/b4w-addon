...
class B4W_JSONExportProcessor(bpy.types.Operator):

    """Export for Blend4Web (.json)"""
    bl_idname = "export_scene.b4w_json_mostfun"
    bl_label = p_("B4W Export Json4mostfun", "Operator")
    ...
    def run(self, export_filepath):
        ...
        html_tpl_path = os.path.join(b4w_webplayer_path, "template", "webplayer_template_mostfun.html")
        # for standalone addon template located in the same dir
        if not os.path.isfile(html_tpl_path):
            html_tpl_path = os.path.join(b4w_webplayer_path, "webplayer_template_mostfun.html")
        ...
        if "CANCELLED" in bpy.ops.export_scene.b4w_json("EXEC_DEFAULT", \
                ...
                is_json4m_export=True, strict_mode=False):
            return
        if (not self.strict_mode or not exporter._b4w_export_errors 
            ...
            try:
                ...
                insertions = dict(built_in_data=data)
                ...
                print("Json file saved to " + export_filepath)
...
class B4W_ExportJSONPathGetter(bpy.types.Operator):
    """Get Export Path for blend file"""
    bl_idname = "b4w.get_export_json_path"
    bl_label = p_("B4W Get Export JSON Path", "Operator")
    bl_options = {'INTERNAL'}

    def execute(self, context):
        print("B4W Export JSON Path = " + get_default_path())

        return {"FINISHED"}

def get_default_path():
    scene = bpy.data.scenes[0]
    if scene.b4w_export_path_json is not "":
...
def set_default_path(path):
    ...
    for i in range(len(bpy.data.scenes)):
        bpy.data.scenes[i].b4w_export_path_json = exporter.guard_slashes(path)
...
def get_smaa_textures(data, json_path):
    b4w_webplayer_path = B4W_JSONExportProcessor.get_b4w_webplayer_path()
...
def b4w_json_export_menu_func(self, context):
    self.layout.operator(B4W_JSONExportProcessor.bl_idname, \
        text=_("Blend4m (.json)")).filepath = get_default_path()

def register():
    bpy.types.INFO_MT_file_export.append(b4w_json_export_menu_func)

def unregister():
    bpy.types.INFO_MT_file_export.remove(b4w_json_export_menu_func)
